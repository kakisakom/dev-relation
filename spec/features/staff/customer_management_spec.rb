require "rails_helper"  #基礎設定の呼び出し

feature "職員による顧客管理" do
  include FeaturesSpecHelper
  let(:staff_member) { create(:staff_member) }  #擬似的に職員を生成
  ...
  scenario "職員が顧客、自宅住所、勤務先を追加する" do
    ...
    within ("fieldset#home-address-fields") do
      fill_in "郵便番号", with: "8888888"
      select "東京都", from: "都道府県"  #ドロップダウンリスト
      fill_in "市区町村", with: "千代田区"
      fill_in "町域、番地等", with: "千代田1-1-1"
      fill_in "建物名、部屋番号等", with: ""
    end
    within ("fieldset#work-address-fields") do
      fill_in "会社名", with: "テスト"   #ここは必須項目
      fill_in "部署名", with: ""
      fill_in "郵便番号", with: ""
      select "", from: "都道府県"
      fill_in "市区町村", with: ""
    ...
    new_customer = Customer.order(:id).last    # 一番最新の登録顧客＝テスト登録顧客
    ...
    expect(new_customer.home_address.postal_code).to eq("8888888")
